...
class HomeScreen(Screen):
    ...
    def update_user_id(self, user_id):
        ...
        self.load_data()  # Reload data with the updated user_id

    def update_username_label(self, username):
        # Find the Label widget with the id 'username_label'
        username_label = self.ids.username_label
        if username_label:
            # Update the text property with the signed-in username
            username_label.text = f'[color=000000][font=Poppins-SemiBold]Hi, {username}![/font][/color]'
    ...
    def load_data(self):
        ...
        cursor.execute(
            'SELECT title, description, date, time FROM tasks WHERE user_id = ? AND date >= ? ORDER BY date, time',
            (self.user_id, current_datetime.strftime('%Y-%m-%d'),))
